...
class ReasoningParser:
    ...
    def should_ignore_prompt_reasoning_end(
        ...
        """Return True when prompt reasoning markers should not set state.

        Some parsers need to classify the first generated bytes for the current
        turn even if previous conversation turns contain reasoning end markers.
        """
        return False
    ...
    def should_force_content_before_reasoning_end(
        ...
        *,
        ...
        """Optionally route early generated text as content.

        Return True to end the implicit reasoning phase and emit
        ``current_text`` as content, False to keep normal reasoning parsing, or
        None to buffer until the next delta.
        """
        return False
