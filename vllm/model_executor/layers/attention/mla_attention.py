...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    def determine_prefill_query_data_type(
        ...
        if use_fp8:
            ...
            disabled_reasons = []
            if dcp_size > 1:
                disabled_reasons.append("decode context parallelism")
            if calculate_kv_scales:
                disabled_reasons.append("runtime KV scale calculation")
            reason = (
                f" This is not currently compatible with {', '.join(disabled_reasons)}."
                if disabled_reasons
                else ""
            )
            logger.info_once(
                ...
                " backend is compatible with FP8 attention.%s",
                reason,
            )
            return model_dtype
        elif (
